...
export const stripeCheckoutSession = catchAsyncErrors(async (req, res, next) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${process.env.FRONTEND_URL}/user/order?order_success=true`,
...
// Create new order after Stipe payment => /api/v1/payment/webhook
  ...
  try {
    ...
    if(event.type === "checkout.session.completed") {
      ...
      const shippingInfo = {
        ...
        phoneNumber: session.metadata.phoneNo
      };
